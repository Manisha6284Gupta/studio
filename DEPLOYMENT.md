...
1.  **Grant Secret Accessor Role:**
    The first time you use secrets, you'll need to grant the Secret Manager Secret Accessor role to the App Hosting service agent. If you haven't run this command before, do so now. Replace `studio-4980755183-3866b` with your Firebase Project ID if it's different:
    ```bash
...
2.  **Create the Google Maps API Key Secret:**
    Run this command to create a secret for your Google Maps API key. Enter your key when prompted:
    ```bash
